...
final_json_path = "D:\\Data\\Code\\script\\script\\json\\data_func"

jsonset_path = "D:\\Data\\Code\\oriBert_compress_202408061923\\oriBert\\corpus\\json2"

# vuljson_path = config.STEP3_DATASET_DIR
# vul_npyset_dir = config.STEP3_VUL_EMBEDDING_DIR + os.sep + config.STEP2_SOFTWARE_NAME

dataset_npyset_dir = "D:\\Data\\Code\\oriBert_compress_202408061923\\oriBert\\npy_data2"
...
def create_features_with_embedding(n_num, json_path, func_name, npyset_dir):
    ...
    json_name = json_path_list[-1].split(".elf")[0]

    npy_path = npyset_dir + os.sep + json_name + os.sep + func_name + ".npy"
...
if __name__ == '__main__':
    ...
    npy_data_list = os.listdir(dataset_npyset_dir)

    for json_file in os.listdir(jsonset_path):
            json_path = jsonset_path + os.sep + json_file
            if json_file.split(".elf")[0] not in npy_data_list:
                continue
            print (json_path)
            with open(json_path) as js:
                for line in js:
                    ...
                    json_npy = json_file.split(".elf")[0]
                    func_name = g_info['fname']
                    npy_path = dataset_npyset_dir + os.sep + json_npy + os.sep + func_name + ".npy"
                    if not os.path.exists(npy_path):
                        continue

                    insts = g_info['features']
                    if g_info['n_num'] < 5 or g_info['n_num'] >= 200:
                        continue
                    ...
                    bit = "64"
                    compiler = "gcc"
                    sofe_name = json_file.split('_')[0].split('-')[0]
                    obfus_type = json_file.split('_')[1].split('-')[-1]
    ...
    with open(final_json_path + os.sep + "func_date2.json" , 'w') as f_out:
